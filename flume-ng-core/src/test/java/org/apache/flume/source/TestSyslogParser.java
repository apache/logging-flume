...
public class TestSyslogParser {
    private static class Entry {
        ...
        public Entry(String rfc5424, String rfc3164, long millis) {
            ...
            this.millis = millis;
            this.localDateTime = LocalDateTime.ofInstant(Instant.ofEpochMilli(millis), ZoneOffset.UTC);
        }
    ...
    public void testParseRfc3164Time() {
        ...
        for (Entry entry : VALID_TIMESTAMPS) {
            ...
            LocalDateTime actual = LocalDateTime.ofInstant(Instant.ofEpochMilli(time), ZoneOffset.UTC);
    ...
    public void testParseRfc5424TimeInvalid() {
        ...
        for (String input : INVALID_RFC5424_TIMESTAMPS) {
            Assert.assertThrows(
                    "Expected parsing to fail for RFC 5424 timestamp '" + input + "'",
