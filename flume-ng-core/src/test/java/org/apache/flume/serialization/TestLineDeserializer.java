...
public class TestLineDeserializer {
    ...
    */
    ...
    private void validateMiniParse(EventDeserializer des) throws IOException {
        ...
        evt = des.readEvent();
        Assert.assertEquals(
                "Line 2 should be repeated, " + "because we reset() the stream", new String(evt.getBody()), "line 2");

        evt = des.readEvent();
        Assert.assertNull("Event should be null because there are no lines " + "left to read", evt);
