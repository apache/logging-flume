...
public class DirectMemoryUtils {
    ...
    public static ByteBuffer allocate(int size) {
        ...
        logger.info("Direct Memory Allocation: " + " Allocation = "
                ...
                + Math.max(0, (maxDirectMemory - allocatedCurrently)));
        try {
        ...
        for (String s : arguments) {
            if (s.contains(MAX_DIRECT_MEMORY_PARAM)) {
                String memSize = s.toLowerCase(Locale.ENGLISH)
                        .replace(MAX_DIRECT_MEMORY_PARAM.toLowerCase(Locale.ENGLISH), "")
                        .trim();
    ...
    private static long getDefaultDirectMemorySize() {
        try {
            ...
            logger.info("Unable to get maxDirectMemory from VM: " + e.getClass().getSimpleName() + ": " + e.getMessage());
        }
