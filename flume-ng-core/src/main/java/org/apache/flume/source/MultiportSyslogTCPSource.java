...
public class MultiportSyslogTCPSource extends SslContextAwareAbstractSource
    ...
    public void configure(Context context) {
        ...
        Preconditions.checkNotNull(portsStr, "Must define config " + "parameter for MultiportSyslogTCPSource: ports");
        ...
        maxEventSize =
                context.getInteger(SyslogSourceConfigurationConstants.CONFIG_EVENTSIZE, SyslogUtils.DEFAULT_SIZE);

        String defaultCharsetStr = context.getString(
                SyslogSourceConfigurationConstants.CONFIG_CHARSET, SyslogSourceConfigurationConstants.DEFAULT_CHARSET);
        try {
        ...
        portCharsets.clear();
        {
            Map<String, String> portCharsetCfg =
                    context.getSubProperties(SyslogSourceConfigurationConstants.CONFIG_PORT_CHARSET_PREFIX);
            for (Map.Entry<String, String> entry : portCharsetCfg.entrySet()) {
                ...
                try {
                    ...
                    throw new IllegalArgumentException(
                            "Unable to parse charset " + "string (" + charsetStr + ") from port configuration.", ex);
                }
            }
        }
        ...
        keepFields = SyslogUtils.chooseFieldsToKeep(context.getString(
    ...
    public void start() {
        ...
        acceptor.setHandler(new MultiportSyslogHandler(
                maxEventSize,
                batchSize,
                getChannelProcessor(),
                sourceCounter,
    ...
    static class MultiportSyslogHandler extends IoHandlerAdapter {
                ...
                for (int num = 0; num < batchSize && buf.hasRemaining(); num++) {
                    ...
                    if (lineSplitter.parseLine(buf, savedBuf, parsedLine)) {
                        ...
                        if (clientIPHeader != null) {
                            event.getHeaders().put(clientIPHeader, SyslogUtils.getIP(session.getRemoteAddress()));
                        }

                        if (clientHostnameHeader != null) {
                            event.getHeaders()
                                    .put(clientHostnameHeader, SyslogUtils.getHostname(session.getRemoteAddress()));
                        }
        ...
        Event parseEvent(ParsedBuffer parsedBuf, CharsetDecoder decoder) {
            ...
            try {
                ...
                event.getHeaders().put(SyslogUtils.EVENT_STATUS, SyslogUtils.SyslogStatus.INVALID.getSyslogStatus());
            ...
            try {
                ...
                if (parsedBuf.incomplete) {
                    event.getHeaders()
                            .put(SyslogUtils.EVENT_STATUS, SyslogUtils.SyslogStatus.INCOMPLETE.getSyslogStatus());
                }
                ...
                if (msgPos == maxLineLength) {
                    ...
                    logger.warn(
                            "Event size larger than specified event size: {}. "
                                    + "Consider increasing the max event size.",
                            maxLineLength);
                    ...
                    throw new IllegalStateException("unexpected buffer state: " + "msgPos="
                            + msgPos + ", buf.hasRemaining=" + buf.hasRemaining() + ", savedBuf.hasRemaining="
                            + savedBuf.hasRemaining() + ", seenNewline="
                            + seenNewline + ", maxLen=" + maxLineLength);
                }
            }
        }
    }
