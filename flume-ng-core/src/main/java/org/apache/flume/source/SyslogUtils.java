...
public class SyslogUtils {
    ...
    public static final String SYSLOG_MSG_RFC3164_0 = "(?:\\<(\\d{1,3})\\>)" + "(?:(\\d)?\\s?)"
            + // version
    ...
    public static String addFieldsToBody(
            Set<String> keepFields, String body, String priority, String version, String timestamp, String hostname) {
    ...
    public SyslogUtils(boolean isUdp) {
        this(DEFAULT_SIZE, new HashSet<>(Arrays.asList(SyslogSourceConfigurationConstants.DEFAULT_KEEP_FIELDS)), isUdp);
    }
    ...
    public void addFormats(Map<String, String> formatProp) {
        ...
        if (formatProp.containsKey(SyslogSourceConfigurationConstants.CONFIG_DATEFORMAT)) {
            fmt1.dateFormat.add(
                    new SimpleDateFormat(formatProp.get(SyslogSourceConfigurationConstants.CONFIG_DATEFORMAT)));
        }
        formats.add(0, fmt1);
    }
    ...
    Event buildEvent() {
        try {
            ...
            if (isBadEvent) {
                ...
                logger.warn(
                        "Event size larger than specified event size: {}. You should "
                                + "consider increasing your event size.",
                        maxSize);
