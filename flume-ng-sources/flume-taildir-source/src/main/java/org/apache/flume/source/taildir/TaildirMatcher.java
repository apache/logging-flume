...
/**
 * Identifies and caches the files matched by single file pattern for <code>TAILDIR<code/> source.
 * <p></p>
 * Since file patterns only apply to the fileNames and not the parent dictionaries, this implementation
 * checks the parent directory for modification (additional or removed files update modification time of parent dir)
 * If no modification happened to the parent dir that means the underlying files could only be written to but no need
 ...
 * <code>System.currentTimeMillis()</code> and <code>File.lastModified()</code>. Also that system clock is used
...
public class TaildirMatcher {
  ...
  // initial value is used in first check only when it will be replaced instantly (system time is positive)
   ...
   * <code>filePattern</code> consists of two parts: first part has to be a valid path to
   * an existing parent directory, second part has to be a
   * valid regex {@link java.util.regex.Pattern} that match any non-hidden file names within parent directory.
  ...
  /**
   * Lists those files within the parentDir that match regex pattern passed in during object instantiation.
   ...
   * It is guaranteed that invocation reflects every change which was observable at the time of invocation.
   * <p></p>
   ...
   * Having said that implementation also stores system time of the previous invocation and previous invocation has to
   ...
   * any further invocations can be served from cache associated with that second (given mtime is not updated again).
   *
   * @return List of files matching the pattern sorted by last modification time. No recursion. No directories.
   * If nothing matches then returns an empty list. If I/O issue occurred then returns the list collected to the point
  ...
  List<File> getMatchingFiles() {
    ...
    // - last mtime change wasn't already checked for sure (system clock hasn't passed that second yet)
   ...
   *
   * Files returned by this call are weakly consistent (see {@link DirectoryStream}). It does not freeze the directory while iterating,
   * so it may (or may not) reflect updates to the directory that occur during the call. In which case next call
  ...
  private List<File> getMatchingFilesNoCache() {
    ...
    try (DirectoryStream<Path> stream = Files.newDirectoryStream(parentDir.toPath(), fileFilter)) {
      ...
      logger.error("I/O exception occurred while listing parent directory. Files already matched will be returned. " +
          parentDir.toPath(), e);
    }
    return result;
  }
   ...
   * Sorting itself use only a snapshot of last modification times captured before the sorting to keep the
