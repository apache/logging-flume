 ...
 * implementation checks the parent directory and subdirectories for modification (additional or removed files
 * update modification time of dir)
...
public class TaildirIncludeChildMatcher implements TailMatcher {
    ...
    TaildirIncludeChildMatcher(String fileGroup, String filePattern, boolean cachePatternMatching) {
        ...
        // The end of the path in the configuration file can be filled with the file regular that needs to be matched
        // Note that if "/" is not written at the end of the path, the end field will be treated as a regular
    ...
    /**
     * Lists those files within the parentDir and subdirectory that match regex pattern passed in during object
     * instantiation. Designed for frequent periodic invocation
     * {@link org.apache.flume.source.PollableSourceRunner}.
     * <p></p>
     * Based on the modification of the parentDirList(parentDir and its subdirectories) this function may trigger cache recalculation by
     * calling {@linkplain #updateMatchingFilesNoCache(File, List<File>)} or
     ...
     * Additionally if a constantly updated directory was configured as parentDir and its subdirectories
     ...
     *
     * Files returned by this call are weakly consistent (new files will be set {@linkplain #lastMatchedFilesCache}).
