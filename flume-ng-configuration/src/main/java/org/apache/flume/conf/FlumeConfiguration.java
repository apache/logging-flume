...
public class FlumeConfiguration {
    ...
    private void validateConfiguration() {
        ...
        logger.info(
                "Post-validation flume configuration contains configuration for agents: {}", agentConfigMap.keySet());
    }
    ...
    public static class AgentConfiguration {
        ...
        private boolean isValid() {
            ...
            if (channelSet.isEmpty()) {
                logger.warn(
                        "Agent configuration for '{}' does not contain any valid channels. " + "Marking it as invalid.",
                        agentName);
                addError(CONFIG_CHANNELS, PROPERTY_VALUE_NULL, ERROR);
                return false;
            }
        ...
        private Pattern createConfigFilterPattern(ConfigFilter configFilter) {
            ...
            return Pattern.compile(
                    "\\$\\{" + // ${
                            Pattern.quote(configFilter.getName())
                            + // <filterComponentName>
                            "\\[(|'|\")"
        ...
        private void filterValue(Context c, String contextKey) {
            for (ConfigFilter configFilter : configFiltersInstances) {
                try {
                    ...
                    e.printStackTrace();
                    logger.error(
                            "Error while matching and filtering configFilter: {} and key: {}",
        ...
        private Set<String> validateChannels(Set<String> channelSet) {
            ...
            while (iter.hasNext()) {
                ...
                if (channelContext != null) {
                    ...
                    ChannelType chType =
                            getKnownChannel(channelContext.getString(BasicConfigurationConstants.CONFIG_TYPE));
                    ...
                    try {
                        conf = (ChannelConfiguration)
                                ComponentConfigurationFactory.create(channelName, config, ComponentType.CHANNEL);
        ...
        private Set<String> validateConfigFilterSet() {
            ...
            while (iter.hasNext()) {
                ...
                if (configFilterContext != null) {
                    ...
                    // Get the configuration object for the channel:
                    ConfigFilterType chType = getKnownConfigFilter(
                            configFilterContext.getString(BasicConfigurationConstants.CONFIG_TYPE));
                    boolean configSpecified = false;
                    String config = null;
                    // Not a known channel - cannot do specific validation to this channel
                    if (chType == null) {
                    ...
                    try {
                        conf = (ConfigFilterConfiguration) ComponentConfigurationFactory.create(
        ...
        private Set<String> validateSources(Set<String> channelSet) {
            ...
            while (iter.hasNext()) {
                ...
                boolean configSpecified = false;
                if (srcContext != null) {
                    SourceType srcType = getKnownSource(srcContext.getString(BasicConfigurationConstants.CONFIG_TYPE));
                    if (srcType == null) {
                    ...
                    try {
                        ...
                        srcConf = (SourceConfiguration)
                                ComponentConfigurationFactory.create(sourceName, config, ComponentType.SOURCE);
                        if (srcConf != null) {
                            ...
                            srcContext.put(CONFIG_CHANNELS, this.getSpaceDelimitedList(channels));
                        }
        ...
        private Set<String> validateSinks(Set<String> channelSet) {
            ...
            while (iter.hasNext()) {
                ...
                if (sinkContext == null) {
                    ...
                    String config = null;
                    boolean configSpecified = false;
                    SinkType sinkType = getKnownSink(sinkContext.getString(BasicConfigurationConstants.CONFIG_TYPE));
                    if (sinkType == null) {
                        config = sinkContext.getString(CONFIG_CONFIG);
                        if (config == null || config.isEmpty()) {
                            config = "OTHER";
                        } else {
                            configSpecified = true;
                    ...
                    try {
                        ...
                        sinkConf = (SinkConfiguration)
                                ComponentConfigurationFactory.create(sinkName, config, ComponentType.SINK);
                        if (sinkConf != null) {
                        ...
                        if (sinkConf != null) errorList.addAll(sinkConf.getErrors());
                        logger.warn(
                                "Could not configure sink  {} due to: {}", new Object[] {sinkName, e.getMessage(), e});
                    }
                }
         ...
         */
        private Set<String> validGroupSinks(
                Set<String> sinkSet, Map<String, String> usedSinks, SinkGroupConfiguration groupConf) {
        ...
        public String getPostvalidationConfig() {
            ...
            if (!sourceConfigMap.isEmpty() || !sinkConfigMap.isEmpty() || !channelConfigMap.isEmpty()) {
