...
/**
 * Avro implementation of a SinlgeMasterRPC. This manages the connection
 * to an Avro server and manages type translation. 
 *
 */
public class AvroMasterRPC implements MasterRPC {
  ...
  AvroMasterRPC(String masterHost, int masterPort) throws IOException {
    ...
    this.masterClient = (FlumeReportAvroServer) 
  ...
  throws IOException {
    try {
      ensureInitialized();
      GenericArray<Utf8> res = masterClient.getLogicalNodes(new Utf8(physNode));
  ...
  throws IOException {
    try {
      ensureInitialized();
      AvroFlumeConfigData config = masterClient.getConfig(new Utf8(n.getName()));
      if (config == null) {
  ...
  public synchronized boolean checkAck(String ackid) throws IOException {
    try {
      ensureInitialized();
      return masterClient.checkAck(new Utf8(ackid));
    } catch (AvroRemoteException e) {
      LOG.debug("Avro error on " + toString(), e);
      throw new IOException(e.getMessage());
    }
  ...
  public synchronized boolean heartbeat(LogicalNode n) throws IOException {
    try {
      ...
      return masterClient.heartbeat(new Utf8(n.getName()), 
          new Utf8(status.physicalNode), new Utf8(status.host), 
          MasterClientServerAvro.stateToAvro(status.state), n.getConfigVersion());
    } catch (AvroRemoteException e) {
      LOG.debug("Avro error on " + toString(), e);
      throw new IOException(e.getMessage());
    }
  ...
  public synchronized void acknowledge(String group) throws IOException {
    try {
      ensureInitialized();
      masterClient.acknowledge(new Utf8(group));
    } catch (AvroRemoteException e) {
      LOG.debug("Avro error on " + toString(), e);
      throw new IOException(e.getMessage());
    }
  ...
  public synchronized void putReports(Map<String, ReportEvent> reports)
    ...
    try {
      ensureInitialized();
      Map<Utf8, FlumeReportAvro> flumeReports = new HashMap<Utf8, FlumeReportAvro>();
      for (Entry<String, ReportEvent> e : reports.entrySet()) {
        flumeReports.put(new Utf8(e.getKey()), ReportServer.reportToAvro(e.getValue()));
      }
