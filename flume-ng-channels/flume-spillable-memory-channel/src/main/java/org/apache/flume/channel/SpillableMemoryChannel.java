...
public class SpillableMemoryChannel extends FileChannel {
    ...
    private double overflowDeactivationThreshold = defaultOverflowDeactivationThreshold / 100;
    ...
    public static class DrainOrderQueue {
        ...
        public void takePrimary(int takeCount) {
            ...
            if (headValue.intValue() < takeCount) {
                throw new IllegalStateException(
                        "Cannot take " + takeCount + " from " + headValue.intValue() + " in DrainOrder Queue");
            }
        ...
        public void takeOverflow(int takeCount) {
            ...
            if (headValue.intValue() > -takeCount) {
                throw new IllegalStateException(
                        "Cannot take " + takeCount + " from " + headValue.intValue() + " in DrainOrder Queue head ");
            }
    ...
    private class SpillableMemoryTransaction extends BasicTransactionSemantics {
        ...
        protected void doPut(Event event) throws InterruptedException {
            ...
            if (!putList.offer(event)) {
                throw new ChannelFullException("Put queue in " + getName() + " channel's Transaction having capacity "
                        + putList.size() + " full, consider reducing batch size of sources");
            }
                ...
                synchronized (queueLock) {
                    ...
                    if (useOverflow) {
                        ...
                        Preconditions.checkNotNull(
                                event, "Queue.poll returned NULL despite" + " semaphore signalling existence of entry");
                    }
                }
                ...
                if (!memQueRemaining.tryAcquire(putList.size(), timeout, TimeUnit.SECONDS)) {
                    if (overflowDisabled) {
                        throw new ChannelFullException(
                                "Spillable Memory Channel's " + "memory capacity has been reached and overflow is "
        ...
        private void commitPutsToPrimary() {
            synchronized (queueLock) {
                ...
                maxMemQueueSize = (memQueue.size() > maxMemQueueSize) ? memQueue.size() : maxMemQueueSize;
                channelCounter.setChannelSize(memQueue.size() + drainOrder.overflowCounter);
            }
    ...
    public void configure(Context context) {
        ...
        if (getLifecycleState() == LifecycleState.START
                || // does not support reconfig when running
        ...
        try {
            byteCapacity = (int)
                    ((context.getLong(BYTE_CAPACITY, defaultByteCapacity) * (1 - byteCapacityBufferPercentage * .01))
                            / avgEventSize);
            if (byteCapacity < 1) {
