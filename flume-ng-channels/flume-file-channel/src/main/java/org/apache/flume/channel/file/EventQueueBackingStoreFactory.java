...
class EventQueueBackingStoreFactory {
    ...
    static EventQueueBackingStore get(File checkpointFile, int capacity, String name, FileChannelCounter counter)
            throws Exception {
    ...
    static EventQueueBackingStore get(
        ...
        try {
            ...
            if (Serialization.VERSION_2 == version) {
                if (upgrade) {
                    return upgrade(
                            checkpointFile, capacity, name, backupCheckpointDir, shouldBackup, compressBackup, counter);
                }
