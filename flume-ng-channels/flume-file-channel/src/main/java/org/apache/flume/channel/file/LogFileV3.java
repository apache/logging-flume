...
public class LogFileV3 extends LogFile {
    ...
    static class MetaDataWriter extends LogFile.MetaDataWriter {
        ...
        protected MetaDataWriter(File logFile, int logFileID) throws IOException {
            ...
            if (version != getVersion()) {
                throw new IOException("Version is " + Integer.toHexString(version) + " expected "
        ...
        void markCheckpoint(long currentPosition, long logWriteOrderID) throws IOException {
             ...
             */
            metaDataBuilder.setBackupCheckpointPosition(logFileMetaData.getCheckpointPosition());
            metaDataBuilder.setBackupCheckpointWriteOrderID(logFileMetaData.getCheckpointWriteOrderID());
        ...
        ProtosFactory.LogFileMetaData read() throws IOException {
            ...
            try {
                ProtosFactory.LogFileMetaData metaData = Preconditions.checkNotNull(
                        ProtosFactory.LogFileMetaData.parseDelimitedFrom(inputStream), "Metadata cannot be null");
     ...
     */
    public static void writeDelimitedTo(Message msg, File file) throws IOException {
    ...
    static class Writer extends LogFile.Writer {
        Writer(
            ...
            if (encryptionKey != null) {
                ...
                Preconditions.checkNotNull(encryptionCipherProvider, "encryptionCipherProvider");
    ...
    static class RandomReader extends LogFile.RandomReader {
        ...
        RandomReader(File file, @Nullable KeyProvider encryptionKeyProvider, boolean fsyncPerTransaction)
                throws IOException {
        ...
        private CipherProvider.Decryptor getDecryptor() {
            ...
            if (decryptor == null) {
                decryptor = CipherProviderFactory.getDecrypter(cipherProvider, key, parameters);
            }
            return decryptor;
        }
    ...
    public static class SequentialReader extends LogFile.SequentialReader {
        ...
        public SequentialReader(File file, @Nullable KeyProvider encryptionKeyProvider, boolean fsyncPerTransaction)
            ...
            try {
                ...
                setPreviousCheckpointWriteOrderID(metaData.getBackupCheckpointWriteOrderID());
            } finally {
                try {
                    inputStream.close();
                } catch (IOException e) {
