...
public class JCEFileKeyProvider extends KeyProvider {
    ...
    public JCEFileKeyProvider(File keyStoreFile, File keyStorePasswordFile, Map<String, File> aliasPasswordFileMap) {
        ...
        try {
            ...
            keyStorePassword =
                    Files.toString(keyStorePasswordFile, Charsets.UTF_8).trim().toCharArray();
    ...
    public Key getKey(String alias) {
        ...
        try {
            ...
            if (aliasPasswordFileMap.containsKey(alias)) {
                ...
                keyPassword =
                        Files.toString(keyPasswordFile, Charsets.UTF_8).trim().toCharArray();
            ...
            String msg = e.getClass().getName() + ": " + e.getMessage() + ". " + "Key = " + alias + ", passwordFile = "
                    + passwordFile;
    ...
    public static class Builder implements KeyProvider.Builder {
        ...
        public KeyProvider build(Context context) {
            String keyStoreFileName = context.getString(EncryptionConfiguration.JCE_FILE_KEY_STORE_FILE);
            String keyStorePasswordFileName =
                    context.getString(EncryptionConfiguration.JCE_FILE_KEY_STORE_PASSWORD_FILE);
            Preconditions.checkState(!Strings.isNullOrEmpty(keyStoreFileName), "KeyStore file not specified");
            Preconditions.checkState(
                    !Strings.isNullOrEmpty(keyStorePasswordFileName), "KeyStore password  file not specified");
            Map<String, File> aliasPasswordFileMap = Maps.newHashMap();
            String passwordProtectedKeys = context.getString(EncryptionConfiguration.JCE_FILE_KEYS);
            Preconditions.checkState(
                    !Strings.isNullOrEmpty(passwordProtectedKeys),
                    "Keys available to KeyStore was not specified or empty");
            for (String passwordName : passwordProtectedKeys.trim().split("\\s+")) {
                String propertyName = Joiner.on(".")
                        .join(
