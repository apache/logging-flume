...
final class MapResolver {
    ...
    public static Map<String, String> resolveProperties(Properties properties) {
        ...
        StringLookup defaultLookup =
                useEnvVars ? new DefaultLookup(map) : StringLookupFactory.INSTANCE.mapStringLookup(map);
        StringLookup lookup =
                StringLookupFactory.INSTANCE.interpolatorStringLookup(createLookupMap(), defaultLookup, false);
        StringSubstitutor substitutor = new StringSubstitutor(lookup);
        substitutor.setEnableSubstitutionInVariables(true);

        properties.stringPropertyNames().forEach((k) -> map.put(k, substitutor.replace(properties.getProperty(k))));
        return map;
    }
    ...
    private static class DefaultLookup implements StringLookup {
        ...
        public String lookup(String key) {
            return properties.containsKey(key) ? properties.get(key) : System.getenv(key);
        }
    }
