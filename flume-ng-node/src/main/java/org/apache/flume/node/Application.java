...
public class Application {
    ...
    private void initializeAllComponents(MaterializedConfiguration materializedConfiguration) {
        ...
        for (SourceRunner sourceRunner :
                materializedConfiguration.getSourceRunners().values()) {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            option = new Option(
                    "f", "conf-file", true, "specify a config file (required if -c, -u, and -z are missing)");
            option.setRequired(false);
            options.addOption(option);

            option = new Option("u", "conf-uri", true, "specify a config uri (required if -c, -f and -z are missing)");
            option.setRequired(false);
            options.addOption(option);

            option = new Option("a", "auth-provider", true, "specify an authorization provider class");
            option.setRequired(false);
            options.addOption(option);
            ...
            option = new Option(
                    "i", "poll-interval", true, "number of seconds between checks for a configuration change");
            option.setRequired(false);
            options.addOption(option);

            option = new Option(
                    "b", "backup-directory", true, "directory in which to store the backup configuration file");
            option.setRequired(false);
            options.addOption(option);
            ...
            if (provider != null) {
                ...
                if (authProviderClass != null) {
                    try {
                        ...
                        Object obj =
                                clazz.getDeclaredConstructor(String[].class).newInstance((Object[]) args);
                ...
                if (authorizationProvider == null
                        && StringUtils.isNotEmpty(confUser)
                ...
                List<LifecycleAware> components = Lists.newArrayList();
                UriConfigurationProvider configurationProvider = new UriConfigurationProvider(
                        agentName, configurationSources, backupDirectory, eventBus, interval);
